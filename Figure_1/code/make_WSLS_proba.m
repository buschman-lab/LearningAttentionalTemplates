function [Model_predictions, u, y] = make_WSLS_proba(Parameters,data,feedback_each_option)
...
for t=1:length(data.IsPrev)
    if data.IsPrev(t)==0
        ...
        if previous_feedback(t) > Parameters.Reward_threshold
            for c=1:N_bins
                Value(c,t)=1-abs(mod(color_binned(c)-previous_chosen_color(t)+pi,2*pi)-pi)/pi; %normalized absolute angular distance to previously chosen color
            end
        else
            Value(1:N_bins,t)=0;
        end
