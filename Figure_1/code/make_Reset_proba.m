function [Model_predictions, u, y] = make_Reset_proba(Parameters,data,feedback_each_option)
...
Reset_fraction=zeros(1,length(data.IsPrev));
...
for t=1:length(data.IsPrev)
    if data.IsPrev(t)==0
        ...
        Weights(:,t)= (1-reset_fraction).*(weights + Parameters.learning_rate*pe*basis_functions(:,prev_chosen_color_bin)) + reset_fraction*(Parameters.learning_rate*previous_feedback(t)*basis_functions(:,prev_chosen_color_bin));
        ...
        %Softmax for choice
        x(t)=rand;
        if x(t)<=Choice_prediction(1,t)
            chosen_id(t)=1;
        elseif x(t)<=Choice_prediction(1,t)+Choice_prediction(2,t)
            chosen_id(t)=2;
        else
            chosen_id(t)=3;
        end
