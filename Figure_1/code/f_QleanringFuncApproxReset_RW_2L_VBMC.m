function [ll, gx] = f_QleanringFuncApproxReset_RW_2L_VBMC(P, u, y, in)
...
Color_pref=mod(P(11),2*pi);
B_color_pref=abs(P(12));
...
%Leanring rate for neagtive RPE
learning_rate_neg = abs(P(13));
...
for t=1:size(chosen_color,2)
    ...
    if isnan(feedback(t)) %1st trial of the sesssion
        ...
        if pe>=0
            ...
            weights(:,t)= (1-reset_fraction).*(weights(:,t-1) + (learning_rate_neg+Precision_effect/(precision+1))*pe*basis_functions(:,chosen_color_bin)) + reset_fraction*((learning_rate_neg+Precision_effect/(precision+1))*feedback(t)*basis_functions(:,chosen_color_bin));
        end
    ...
    for l=1:3
        choice_value(l,1)=value_new(options_color_bin(l))+...
