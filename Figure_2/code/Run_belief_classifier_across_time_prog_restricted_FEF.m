function Run_belief_classifier_across_time_prog_restricted_FEF(fsroot, ROI, n_tt, this_time)
...
save_name=sprintf('Pseudo_pop_peak_belief_prog_across_time_results_more_trials_%s_%d_%d',ROI,this_time,n_tt);
...
for k=1:N_belief
    for n=1:N_neurons
        for t=1:N_data
            for p=1:N_prog
                data_this_belief(n,t+(p-1)*N_data)=this_ROI.Pseudo_pop(this_ROI.Neurons(n_tt,n),this_ROI.progression.TT_trials(n_tt,n,t,p,k)).Classifier_FR(this_time); %%error Val and TT are inverted
            end
        end
    end
        ...
        if l~=k
            for n=1:N_neurons
                for t=1:N_data/(N_belief-1)
                    for p=1:N_prog
                        data_other_belief(n,t+increment*N_data/(N_belief-1)+(p-1)*N_data)=this_ROI.Pseudo_pop(this_ROI.Neurons(n_tt,n),this_ROI.progression.TT_trials(n_tt,n,other_belief_ind(t),p,l)).Classifier_FR(this_time);
                    end
                end
            end
            increment=1;
...
for tp=1:N_times
    for q=1:N_prog
        ...
        for i=1:length(Classifier_proba)
            [Classification_this_label(tp,q,i), Classification_label(tp,q,i)]=max(Classifier_proba(i,:));
            Classification_correct(tp,q,i)=(Classification_label(tp,q,i)==groups_val_this_belief(i));
        end
