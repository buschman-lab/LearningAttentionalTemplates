function Run_true_template_classifier_combined_time_prog_restricted_FEF(fsroot, ROI, n_tt, this_combination)
...
boot_name='Pseudo_pop_true_template_prog_classifier_bootstrap_more_trials'; %added peak
if this_combination==1
    save_name=sprintf('Pseudo_pop_true_template_prog_combined_time_results_more_trials_%s_600ms_%d',ROI,n_tt);
elseif this_combination==2
    save_name=sprintf('Pseudo_pop_true_template_prog_combined_time_results_more_trials_%s_900ms_%d',ROI,n_tt);
end
...
N_template=size(this_ROI.progression.TT_trials,5);
...
for k=1:N_template
    ...
    other_template_ind=randsample(N_data,N_data/(N_template-1),'false');
    increment=0;
    for l=1:N_template
        if l~=k
            for n=1:N_neurons
                for t=1:N_data/(N_template-1)
                    for p=1:N_prog
                        if this_combination==1
                            data_other_template(n,t+increment*N_data/(N_template-1)+(p-1)*N_data)=(this_ROI.Pseudo_pop(this_ROI.Neurons(n_tt,n),this_ROI.progression.TT_trials(n_tt,n,other_template_ind(t),p,l)).Classifier_FR(8)+this_ROI.Pseudo_pop(this_ROI.Neurons(n_tt,n),this_ROI.progression.TT_trials(n_tt,n,other_template_ind(t),p,l)).Classifier_FR(14))/2;
                        elseif this_combination==2
                            data_other_template(n,t+increment*N_data/(N_template-1)+(p-1)*N_data)=(this_ROI.Pseudo_pop(this_ROI.Neurons(n_tt,n),this_ROI.progression.TT_trials(n_tt,n,other_template_ind(t),p,l)).Classifier_FR(8)+this_ROI.Pseudo_pop(this_ROI.Neurons(n_tt,n),this_ROI.progression.TT_trials(n_tt,n,other_template_ind(t),p,l)).Classifier_FR(14)+this_ROI.Pseudo_pop(this_ROI.Neurons(n_tt,n),this_ROI.progression.TT_trials(n_tt,n,other_template_ind(t),p,l)).Classifier_FR(20))/3;
                        end
                    end
                end
                increment=1;
    ...
    data_classifier=[data_this_template,data_other_template]';
    ...
    clear c cv *_this_template *_classifier labels probas scores mdl *_other_template*
end
...
for q=1:N_prog
    ...
    for l=1:N_template
        for t=1:size(this_ROI.progression.Val_trials,3)
            ...
            groups_val_this_template(t+(l-1)*size(this_ROI.progression.Val_trials,3))=l;
        end
    end
    for k=1:N_template
        [~, probas,~] = predict(SVMModel(k,1).mdl,data_val_this_template');
    ...
    for i=1:length(Classifier_proba)
        ...
        Classification_correct(q,i)=(Classification_label(q,i)==groups_val_this_template(i));
    end
    clear Classifier_proba *_this_template
end
