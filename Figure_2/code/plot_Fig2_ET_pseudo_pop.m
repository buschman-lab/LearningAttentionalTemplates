...
if this_combination==1
    save_name=sprintf('Pseudo_pop_peak_belief_prog_combined_time_results_more_trials_%s_600ms_proj','PFC');
elseif this_combination==2
    save_name=sprintf('Pseudo_pop_peak_belief_prog_combined_time_results_more_trials_%s_900ms_proj','PFC');
end
...
for n_tt=1:100
    ...
    if this_combination==2
        results_name=sprintf('Pseudo_pop_peak_belief_prog_combined_time_results_more_trials_%s_900ms_%d',ROI,n_tt);
        results_NN_name=sprintf('Pseudo_pop_peak_belief_prog_combined_time_results_more_trials_%s_900ms_NN_%d',ROI,n_tt);
        results_NN_each_prog_name=sprintf('Pseudo_pop_peak_belief_each_prog_combined_time_results_more_trials_%s_900ms_NN_%d',ROI,n_tt);
        results_proj_name=sprintf('Pseudo_pop_peak_belief_prog_combined_time_results_more_trials_%s_900ms_%d_proj',ROI,n_tt);
    end
...
%%
for k=1:3
    p_LIP(k,1)=z_test_function_bootstrap(LIP_mean_classification_net(k,:),1/3);
    p_LIP(k,2)=z_test_function_bootstrap(LIP_mean_classification_net_each_prog(k,:),1/3);
    p_FEF(k,1)=z_test_function_bootstrap(FEF_mean_classification_net(k,:),1/3);
    p_FEF(k,2)=z_test_function_bootstrap(FEF_mean_classification_net_each_prog(k,:),1/3);
    p_PFC(k,1)=z_test_function_bootstrap(PFC_mean_classification_net(k,:),1/3);
    p_PFC(k,2)=z_test_function_bootstrap(PFC_mean_classification_net_each_prog(k,:),1/3);
end


for tp=1:length(window_start_list)
    p_LIP_across_time(tp)=z_test_function_bootstrap(LIP_across_time(tp,:),1/3);
    p_FEF_across_time(tp)=z_test_function_bootstrap(FEF_across_time(tp,:),1/3);
    p_PFC_across_time(tp)=z_test_function_bootstrap(PFC_across_time(tp,:),1/3);
end
