function models_FR_figure2_true_template(fsroot, ROI, arrayID, this_time)
...
savename=sprintf('models_FR_figure2_true_template_%s_%d',ROI,arrayID);
    ...
    switch event
        ...
        case 'reward_end'
            for i=1:length(Conditions.stim)-1
                ...
                true_template(i)=true_template(i+1);
            end
            ...
            true_template(length(Conditions.stim))=NaN;
...
%             Belief_this_neuron=belief_probability(selected,:);
            ...
            true_template_this_neuron=true_template(selected)';
            fr=fr(selected);
            
            
            fr=zscore(fr);
            
            [R2(count_ROI,:,:),R2_baseline(count_ROI,:),param(count_ROI,:,:,:),partition_saved(count_ROI)] = compute_variance_explained(true_template_this_neuron, fr',N_param, N_fold, N_models);
            
            for np=1:N_perm
                fr = fr(randperm(length(fr)));
                
                [R2_null(count_ROI,:,:,np),R2_baseline_null(count_ROI,:,np),~,~] = compute_variance_explained(true_template_this_neuron, fr',N_param, N_fold, N_models);
    ...
    if exist('R2')>0
        save(fullfile(data_path,savename),'R2','R2_baseline','R2_diff','param','partition_saved','Neuron_id','Session_id','R2_null','R2_baseline_null','-v7.3')
    end
...
function [R2,R2_baseline,param,c,exitflag] = compute_variance_explained(Template, y, Nparam, N_fold, N_models)

c = cvpartition(length(y),'KFold',N_fold);

for np=1:N_fold
    
    [R2(:,np),R2_baseline(np),param(:,:,np),exitflag(:,np)]=regf(Template(c.training(np)), y(c.training(np)),...
        Template(c.test(np)), y(c.test(np)), Nparam,N_models);
    
end

    function [R2,R2_baseline,param,exitflag] = regf(Template, y,...
            Template_test, ytest, Nparam,Nmodels)
        
        R2=zeros(Nmodels,1);
        param=zeros(Nparam,Nmodels);
        
        %Template
        [R2(1),param(1:4,1),exitflag(1)] = regf_stim(Template,y,Template_test,ytest);
