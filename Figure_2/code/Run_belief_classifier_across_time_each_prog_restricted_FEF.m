function Run_belief_classifier_across_time_each_prog_restricted_FEF(fsroot, ROI, n_tt, this_time)
...
save_name=sprintf('Pseudo_pop_peak_belief_each_prog_across_time_results_more_trials_%s_%d_%d',ROI,this_time,n_tt);
...
Classification_label=NaN(N_times,N_prog,N_data_val,N_boot);
Classification_this_label=NaN(N_times,N_prog,N_data_val,N_boot);
Classification_correct=NaN(N_times,N_prog,N_data_val,N_boot);

for p=1:N_prog
    for k=1:N_belief
        for n=1:N_neurons
            for t=1:N_data
                data_this_belief(n,t)=this_ROI.Pseudo_pop(this_ROI.Neurons(n_tt,n),this_ROI.progression.TT_trials(n_tt,n,t,p,k)).Classifier_FR(this_time); %%error Val and TT are inverted
            end
        end
            ...
            if l~=k
                for n=1:N_neurons
                    for t=1:N_data/(N_belief-1)
                            data_other_belief(n,t+increment*N_data/(N_belief-1))=this_ROI.Pseudo_pop(this_ROI.Neurons(n_tt,n),this_ROI.progression.TT_trials(n_tt,n,other_belief_ind(t),p,l)).Classifier_FR(this_time);
                    end
                end
                increment=1;
            end
    ...
    for p=1:N_prog
        ...
        for l=1:N_belief
            for t=1:size(this_ROI.progression.Val_trials,3)
                for n=1:N_neurons
                    data_val_this_belief(n,t+(l-1)*size(this_ROI.progression.Val_trials,3))=this_ROI.Pseudo_pop(this_ROI.Neurons(n_tt,n),this_ROI.progression.Val_trials(n_tt,n,t,p,l)).Classifier_FR(tp);  %%error Val and TT are inverted
                end
        ...
        for i=1:length(Classifier_proba)
            [Classification_this_label(tp,p,i), Classification_label(tp,p,i)]=max(Classifier_proba(i,:));
            Classification_correct(tp,p,i)=(Classification_label(tp,p,i)==groups_val_this_belief(i));
        end
