function Run_belief_classifier_combined_time_prog_restricted_FEF_noOne(fsroot, ROI, n_tt, this_combination)
...
if this_combination==1
    save_name=sprintf('Pseudo_pop_peak_belief_prog_combined_time_results_noOne_%s_600ms_%d',ROI,n_tt);
elseif this_combination==2
    save_name=sprintf('Pseudo_pop_peak_belief_prog_combined_time_results_noOne_%s_900ms_%d',ROI,n_tt);
end
...
%check how many blocks there are in each bin and for each neuron
    for nn=1:N_neurons
        for n_c=1:N_belief
            for i=1:size(this_ROI.progression.TT_trials,3)
                Block_TT(i,nn,n_c)=this_ROI.Pseudo_pop(this_ROI.Neurons(n_tt,nn),this_ROI.progression.TT_trials(n_tt,nn,i,3,n_c)).Block_nb; %third level of preogression to be super conservative
            end
...
for nn=1:N_neurons
    for n_c=1:N_belief
        Unique_block(nn,n_c)=length(unique(Block_TT(:,nn,n_c)));
    end
end
%if there is only one block for any bin for a neuron then remove it
for nn=1:N_neurons
    Remove_neuron(nn)=sum(Unique_block(nn,:)==1)==0;
end
...
for k=1:N_belief
        ...
        if l~=k
            for n=1:N_neurons
                for t=1:N_data/(N_belief-1)
                    for p=1:N_prog
                        if this_combination==1
                            data_other_belief(n,t+increment*N_data/(N_belief-1)+(p-1)*N_data)=(this_ROI.Pseudo_pop(this_ROI.Neurons(n_tt,n),this_ROI.progression.TT_trials(n_tt,n,other_belief_ind(t),p,l)).Classifier_FR(8)+this_ROI.Pseudo_pop(this_ROI.Neurons(n_tt,n),this_ROI.progression.TT_trials(n_tt,n,other_belief_ind(t),p,l)).Classifier_FR(14))/2;
                        elseif this_combination==2
                            data_other_belief(n,t+increment*N_data/(N_belief-1)+(p-1)*N_data)=(this_ROI.Pseudo_pop(this_ROI.Neurons(n_tt,n),this_ROI.progression.TT_trials(n_tt,n,other_belief_ind(t),p,l)).Classifier_FR(8)+this_ROI.Pseudo_pop(this_ROI.Neurons(n_tt,n),this_ROI.progression.TT_trials(n_tt,n,other_belief_ind(t),p,l)).Classifier_FR(14)+this_ROI.Pseudo_pop(this_ROI.Neurons(n_tt,n),this_ROI.progression.TT_trials(n_tt,n,other_belief_ind(t),p,l)).Classifier_FR(20))/3;
                        end
                    end
                end
                increment=1;
    ...
    %remove neurons
    data_classifier=data_classifier(:,Remove_neuron);
...
for q=1:N_prog
    ...
    %remove neurons
    data_val_this_belief=data_val_this_belief(Remove_neuron,:);
    for k=1:N_belief
