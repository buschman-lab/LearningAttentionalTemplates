function models_FR_figure2_value_function(fsroot, ROI, arrayID, this_time)
...
savename=sprintf('models_FR_figure2_value_function_%s_%d',ROI,arrayID);
...
%             Belief_this_neuron=belief_probability(selected,:);
            ...
            switch event
                case 'target'
                    [R2(count_ROI,:,:),R2_baseline(count_ROI,:),param(count_ROI,:,:,:),partition_saved(count_ROI)] = compute_log_model_evidence(chosen_color_this_neuron, ...
...
function [R2,R2_baseline,param,c,exitflag] = compute_log_model_evidence(Chosen_color, Belief, Peak_belief, Mean_value, y, Nparam, N_fold, N_models)
