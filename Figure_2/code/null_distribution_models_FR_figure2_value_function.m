function null_distribution_models_FR_figure2_value_function(fsroot, ROI, arrayID, this_time)
...
savename=sprintf('null_distribution_models_FR_figure2_value_function_%s_%d',ROI,arrayID);
...
if strncmp(ROI,'LIP',3) && arrayID==34
    ...
    for n=1:size(this_ROI,1)
        ...
        if sum(~isnan(this_ROI(n,:,1)),2)>500
            ...
            for np=1:N_perm
                ...
                switch event
                    case 'target'
                        [R2_null(count_ROI,:,:,np),R2_baseline_null(count_ROI,:,np),~,~] = compute_variance_explained(chosen_color_this_neuron, ...
                            Belief_this_neuron, Peak_belief_this_neuron, Mean_value_this_neuron, fr',N_param, N_fold, N_models);
                    case 'reward_end'
                        [R2_null(count_ROI,:,:,np),R2_baseline_null(count_ROI,:,np),~,~] = compute_variance_explained(prev_chosen_color_this_neuron, ...
                            Belief_this_neuron, Peak_belief_this_neuron, Mean_value_this_neuron, fr',N_param, N_fold, N_models);
                end
                for j=1:size(R2_null,2)
                    for k=1:size(R2_null,3)
                        R2_diff_null(count_ROI,j,k,np)=R2_null(count_ROI,j,k,np)-R2_baseline_null(count_ROI,k,np);
                    end
                end
    ...
    if exist('R2_null')>0
        save(fullfile(data_path,savename),'R2_null','R2_baseline_null','R2_diff_null','Neuron_id','Session_id','-v7.3')
    end
...
function [R2,R2_baseline,param,c,exitflag] = compute_variance_explained(Chosen_color, Belief, Peak_belief, Mean_value, y, Nparam, N_fold, N_models)
...
for np=1:N_fold
    ...
    [R2(:,np),R2_baseline(np),param(:,:,np),exitflag(:,np)]=regf(Chosen_color(c.training(np)), Belief(c.training(np),:), Peak_belief(c.training(np)), Mean_value(c.training(np)), y(c.training(np)),...
        Chosen_color(c.test(np)), Belief(c.test(np),:), Peak_belief(c.test(np)), Mean_value(c.test(np)), y(c.test(np)), Nparam,N_models);
    
end

    function [R2,R2_baseline,param,exitflag] = regf(Chosen_color, Belief, Peak_belief, Mean_value, y,...
            Chosen_color_test,  Belief_test, Peak_belief_test, Mean_value_test, ytest, Nparam,Nmodels)
        ...
        %Chosen color
        [R2(4),param(1:4,1),exitflag(4)] = regf_stim(Chosen_color,y,Chosen_color_test,ytest);
